...
class Base
    ...
    loop do
     ...
     "
      ...
      case answer
        ...
        when 7 then manage_wagon
        when 8 then forward
        when 9 then backward
        when 10 then trains_on_station
        when 11 then all_stations_and_trains
        when 12 then break
      end
  end
end
  ...
  # интерфейс занимания мест в вагонах
  def manage_wagon
    p "Choose train:"
    @trains.each_with_index {|train, index|p "#{index + 1}. #{train.number}"}
  ...
  # выодит информацию о поездах и вагонах на станции в соответствии с требованиями последнего задания
